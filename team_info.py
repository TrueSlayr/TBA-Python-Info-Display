...
events = tba.team_events(team_number_appended, 2020)
...
if team_number_appended == str("frc") + (team_number_provided): # Check whether string for Team Number is formatted properly
    ...
    print('In 2020, Team ' + team_number + ' participated at %d events: %s.' % (len(events), ', '.join(event.event_code for event in events))) # Print Team Events
